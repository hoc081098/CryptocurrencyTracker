...
class TickersAdapter(private val onClickListener: (position: Int) -> Unit) :
    ListAdapter<ModelTicker, androidx.recyclerview.widget.RecyclerView.ViewHolder>(diffCallback) {
    ...
    private class LoadMoreViewHolder(itemView: View) :
    ...
    private class TickerViewHolder(
        ...
        fun bindTo(viewTicker: ModelTicker.Ticker?) = containerView.run {
            ...
            text_name.text =
                context.getString(R.string.view_ticker_name, viewTicker?.rank, viewTicker?.name)
            ...
            Picasso.get()
                .load("https://res.cloudinary.com/dxi90ksom/image/upload/${viewTicker?.symbol}.png")
                .fit()
                .centerCrop()
